# Runbook Generator — Standalone Real GUI Implementation
...
Default URL: `http://127.0.0.1:9149`

## What is inside this project folder

- `app/` — FastAPI backend for this project.
- `static/` — elegant browser GUI.
- `plugins/runbook-generator.json` — this project’s own feature/customization/input schema.
...
- `cloud_stack` — cloud stack (text)
- `service_tier` — service tier (text)
- `slos` — SLOs (text)
- `team_contacts` — team contacts (text)
- `severity_model` — severity model (select)
- `escalation_policy` — escalation policy (text)
- `runbook_format` — runbook format (select)
...
- `service_architecture` — Service architecture (text) required
