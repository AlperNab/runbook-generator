...
**Domain:** `DevOps / SRE` · **Suite:** `Developer Productivity Suite` · **Accent:** `#60a5fa`
...
**Runbook Generator** is a standalone, browser-based AI workflow app for **DevOps / SRE**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.

**Core job:** Service architecture → operational runbook.
...
|---|---:|:---:|---|---|
| `service_architecture`<br>Service architecture | text | Yes | — | Affects input: Service architecture. |
...
| `cloud_stack`<br>cloud stack | text | No | — | Affects customization: cloud stack. |
| `service_tier`<br>service tier | text | No | — | Affects customization: service tier. |
| `slos`<br>SLOs | text | No | — | Affects customization: SLOs. |
| `team_contacts`<br>team contacts | text | No | — | Affects customization: team contacts. |
| `severity_model`<br>severity model | select | No | CVSS | Affects customization: severity model. |
| `escalation_policy`<br>escalation policy | text | No | — | Affects customization: escalation policy. |
| `runbook_format`<br>runbook format | select | No | — | Affects customization: runbook format. |
...
uvicorn app.main:app --host 127.0.0.1 --port 9149
```
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9149`. |
...
```bash
curl http://127.0.0.1:9149/api/health
```
...
```bash
curl -X POST http://127.0.0.1:9149/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9149 uvicorn app.main:app --host 0.0.0.0 --port 9149
```
...
- `plugins/runbook-generator.json` for plugin metadata.
